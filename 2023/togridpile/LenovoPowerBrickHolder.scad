...
//// TGx9.3, minus the top-level program
...
function tgx9_get_unit_table() = $tgx9_unit_table;
...
function decode_dim(dim_ca) = is_num(dim_ca) ? dim_ca : tog_unittable__divide_ca($tgx9_unit_table, dim_ca, [1, "mm"]);
function decode_dims(vec_ca) = [ for(dim_ca=vec_ca) decode_dim(dim_ca) ];
...
cord_slot_width = decode_dim(cord_slot_width_ca);
cord_slot_depth = decode_dim(cord_slot_depth_ca);

module tgx9_usermod_2(what) {
	if( what == "brick-cavity" ) {
		brick_size = decode_dims(brick_size_ca);
