...
//
// v0.2:
// - cap_diameter can be overridden
...
// Leave blank for default based on part_type
cap_diameter = "";
...
post =
	...
	tphl1_make_z_cylinder(zds=[
		[cap_height_mm - 0.1 , d0+2    ],
		[cap_height_mm + 0.9 , d0      ],
		[15-bev              , d0      ],
...
function make_single_cap(d, h=3.1, bev=0.7) =
	tphl1_make_z_cylinder(zds=[
		[   0 , d-bev*2],
		[  bev, d      ],
		[h-bev, d      ],
...
function make_double_cap(d, h=3.1, tbev=0.3) =
	let( bbev = h-tbev-1 )
	tphl1_make_z_cylinder(zds=[
		[0       , d-bbev*2],
		[h-tbev-1, d       ],
...
part =
	part_type == "SinglePlug" ? ["union",
		post,
		make_single_cap(
			d = cap_diameter == "" ? inch/2 : togunits1_to_mm(cap_diameter),
			h = cap_height_mm
		)
	] :
	part_type == "DoublePlug" ? ["union",
		["translate", [-6.35,0,0], post],
		["translate", [ 6.35,0,0], post],
		make_double_cap(
			d = cap_diameter == "" ? inch*(1+3/32) : togunits1_to_mm(cap_diameter),
			h = cap_height_mm
		)
	]:
